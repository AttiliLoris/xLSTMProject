...
def main():
    ...
    with open(
            "nostriEsperimenti/output.txt", 'w') as f:
    ...
    for batch_size in batch_sizes:
        for lr in lrs:
            ...
            for epochs in [n_epochs]:
                ...
                with open(
                        "nostriEsperimenti/output.txt", 'a') as f:
                    print(correct, total, file=f)
                    print(classification_report(all_labels, all_predictions, labels=labels,
            ...
            plt.grid(True)
            plt.savefig(
                f"{current_script_dir}/loss_graphics/grafico_loss_{batch_size}_{lr}.png",
                format='png', dpi=300)
            plt.close()
