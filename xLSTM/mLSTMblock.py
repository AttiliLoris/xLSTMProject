...
class mLSTMblock(nn.Module):
    def __init__(self, x_example, factor, depth, kernel_size, dropout=0.2):
        super().__init__()
        self.input_size = x_example.shape[2]
        self.hidden_size = int(self.input_size*factor)
        self.bs = x_example.shape[0]
        ...
        self.conv = CausalConv1D(self.hidden_size, self.hidden_size, kernel_size)#modificato
    ...
    def init_states(self, x_example):
        self.ct_1 = torch.zeros([self.bs, x_example.shape[1], self.hidden_size], device=x_example.device)
        self.nt_1 = torch.zeros([self.bs, x_example.shape[1], self.hidden_size], device=x_example.device)
